...
### Shielding — spectrum-weighted narrow beam (all other nuclides)
T(x) = Σ wᵢ · e^(−μ(Eᵢ)x)

wᵢ = relative H\*(10) dose contribution of each photon line (ICRP 107 yields × ICRU 57, normalized; stored as `shielding_spectrum` in `data/nuclides.json`, generated by `tools/add-shielding-spectra.js`). This captures **beam hardening** of multi-line emitters — a single-line exponential can underestimate the transmitted dose by orders of magnitude behind thick shields (e.g. Ga-67 TVL in Pb: 0.4 mm single-line vs 4.2 mm spectrum-weighted). No build-up factor is applied, so thick-shield doses may still be underestimated. μ from NIST XCOM; lead K-edge at 88 keV handled explicitly. Single-line narrow beam remains the fallback for custom nuclides without a stored spectrum.
